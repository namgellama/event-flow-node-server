...
export async function findByEventAndUser(
    ...
    const [eventRecipient] = await db
        ...
        .where(
            and(eq(eventRecipientsTable.eventId, eventId), eq(eventRecipientsTable.userId, userId)),
        );
...
export async function getByEventId(eventId: string): Promise<EventRecipient[]> {
    return db.select().from(eventRecipientsTable).where(eq(eventRecipientsTable.eventId, eventId));
}
...
export async function markSending(eventId: string, userId: string): Promise<void> {
    await db
...
export async function markFailed(eventId: string, userId: string): Promise<void> {
    await db
        .update(eventRecipientsTable)
        .set({
            status: "FAILED",
