...
export async function getAllEvents(req: Request, res: Response, next: NextFunction) {
    ...
    try {
        const { events, pagination } = await eventService.getAll(page, pageSize);

        sendResponse(res, { events, pagination }, "Events fetched successfully");
    } catch (error) {
        next(error);
    }
}
...
export async function updateEvent(req: Request<{ id: string }>, res: Response, next: NextFunction) {
    try {
